...
public class IRQueueCountPageTest extends TestBase {
	
	//Define Variable(s)
	SoftAssert checkpoint;
	
	//Constructor
	public IRQueueCountPageTest() {
		super();
	}
	...
	public void beforeClass(String excelPath) {
		...
		reportLogger = report.startTest("IRQueueCountPageTest Script");
		...
		irQueueCountPage = new IRQueueCountPage(eDriver, reportLogger);
	}
	
	//Test the login functionality
	@Test(dataProvider="inputs", dataProviderClass=ExcelMethods.class)
	public void irQueueCountPageTest(String active, String environment, String queueType, String searchItemCount, String queueCount, String queueCountDifference, String irSearchItemCount, String irQueueCount, String irQueueCountDifference, String finalResult, String dataRow) {
		System.out.println("@Test - irQueueCountPageTest()");
		...
		if (active.equalsIgnoreCase("y") || active.equalsIgnoreCase("yes")) {
			...
			mainPage.navigateToIRQueuePage(environment);
			
			if (queueType.equalsIgnoreCase("queues - all")) {
				//Check if the user is on the correct branch
				checkpoint = irQueueCountPage.verifySearchCapabilities(checkpoint);
			} else {
				//Select the relevant Queue Type
				irQueueCountPage.selectQueueType(queueType);
				
				//Pause the script for a bit
				genMethods.waitFor(2);
				
				//Click the 'Search' button
				irQueueCountPage.clickSearchButton();
				
				//Pause the script for a bit
				genMethods.waitFor(2);
				
				//Retrieve the number of 'Search' results
				searchCountResult = irQueueCountPage.getSearchCount();
				
				//Click the 'Queue Count' button
				irQueueCountPage.clickQueueCount();
				
				//Retrieve the number of 'Queue Count' results
				queueCountResult = irQueueCountPage.getQueueCount();
				...
				excelMethods.setDataTableCell(searchCountResult, iteration, 6);
				excelMethods.setDataTableCell(queueCountResult, iteration, 7);
				excelMethods.setDataTableCell(resultsCountDifference, iteration, 8);
				
				//Check if the results difference is less than 10
				checkpoint = irQueueCountPage.verifyQueueDifference(checkpoint, resultsCountDifference);
			}
