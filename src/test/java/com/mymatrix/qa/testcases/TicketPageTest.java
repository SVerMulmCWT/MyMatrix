...
public class TicketPageTest extends TestBase {
	...
	public void beforeClass(String excelPath) {
		...
		excelMethods.setSheetName("Tickets");
		column = 8;
	...
	public void ticketPageTest(String active, String environment, String clientFileNumber, String documentType, String documentNumber, String paymentType, String baseCost, String finalResult, String dataRow) {
		...
		if (active.equalsIgnoreCase("y") || active.equalsIgnoreCase("yes")) {
			...
			foundTicket = clientFilePage.navigateToExistingTicketDetailPage(environment, documentNumber);
			
			//Check if the Ticket info is correct
			if (foundTicket && documentType.equalsIgnoreCase("ticket")) {
				checkpoint = bspTicketPage.verifyTicketDetailPage(checkpoint, paymentType, baseCost);
			} else if (foundTicket && documentType.equalsIgnoreCase("service fee - document")) {
				checkpoint = bspTicketPage.verifyServiceFeeDocumentDetailPage(checkpoint, paymentType, baseCost);
			} else {
				checkpoint.assertEquals("Ticket not found", "Ticket found");
			}
